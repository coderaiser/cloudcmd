    ...
    if (scope.window) {
     ...
     function CloudFuncProto(Util) {
        ...
        */
         ...
         */
        this._getFileTableHeader           = function(pFileTableTitles) {
            var i, lStr, lHeader='<li class=fm-header>';
            
            lHeader+='<span class=mini-icon></span>';
            
            for(i = 0;i < pFileTableTitles.length; i++) {
                lStr     = pFileTableTitles[i];
        ...
        this.buildFromJSON                 = function(pJSON, pTemplate, pPathTemplate) {
            var lFile, type, link, target, size, owner, mode,
                files = pJSON.files,
