...
(function(Util, DOM){
        ...
        RESTfull        = function(){
           ...
           function sendRequest(pParams){
                ...
                if(lRet){
                    ...
                    DOM.Images.showLoad( p.imgPosition );
     ...
     */
    DOM.createEvent             = function(pEventName, pKeyCode){
     ...
     * @param pKeyCode
     */
    DOM.createKeyEvent          = function(pKeyCode){
        return DOM.createEvent('keydown', pKeyCode);
    };
     ...
     * @param pKeyCode
     */
    DOM.createClickEvent          = function(){
        return DOM.createEvent('click');
    };
     ...
     * @param pKeyCode
     */
    DOM.createDblClickEvent          = function(){
        return DOM.createEvent('dblclick');
    };
     ...
     * @param pEvent
     */
    DOM.dispatch                = function(pEvent, pElement){
     ...
     * @param pElement
     */
    DOM.dispatchKeyEvent        = function(pKeyCode, pElement){
        var lEvent  = DOM.createKeyEvent(pKeyCode),
            lRet    = DOM.dispatch(lEvent, pElement);
     ...
     * @param pElement
     */
    DOM.dispatchClickEvent        = function(pElement){
        var lEvent  = DOM.createClickEvent(),
            lRet    = DOM.dispatch(lEvent, pElement);
     ...
     * @param pElement
     */
    DOM.dispatchDblClickEvent        = function(pElement){
        var lEvent  = DOM.createDblClickEvent(),
            lRet    = DOM.dispatch(lEvent, pElement);
    ...
    DOM.promptDeleteSelected = function(pCurrentFile){
        var lRet,
            ...
            lMsgSel     = 'selected ',
            lFiles      = DOM.getSelectedFiles(),
            lSelected   = DOM.getSelectedNames(lFiles),
        ...
        if(n > 1){
            ...
            lIsDir = DOM.isCurrentIsDir(lCurrent);
     ...
     * @param pCurrentFile
     */
    DOM.isCurrentFile           = function(pCurrent){
        var lRet;
        
        if( pCurrent )
            lRet = DOM.isContainClass(pCurrent, CURRENT_FILE);
    ...
    DOM.isSelected              = function(pSelected){
        ...
        if( pSelected )
            lRet = DOM.isContainClass(pSelected, SELECTED_FILE);
    ...
    DOM.isCurrentIsDir            = function(pCurrent){
        var lCurrent    = pCurrent || DOM.getCurrentFile(),
            ...
            lRet        = DOM.isContainClass(lFileType, 'directory');
   ...
   /**
    ...
    DOM.getSelectedNames        = function(pSelected){
        var lSelected   = pSelected || DOM.getSelectedFiles(),
            lRet        = lSelected ? [] : null;
            
        if(lRet){
            var lFirst  = lSelected[0],
                lName   = DOM.getCurrentName( lFirst );
                
            if(lName === '..')
                DOM.unsetSelectedFile( lFirst );
            
            for(var i = 0, n = lSelected.length; i < n;i++)
                lRet[i] = DOM.getCurrentName( lSelected[i] );
        }
        return lRet;
    };
