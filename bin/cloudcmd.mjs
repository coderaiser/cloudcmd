...
async function main() {
    const args = yargs
        ...
        // listener
        .option('listen', {
            describe: 'Set listening address, must be a valid URL' +
    ...
    if (args.server) {
        const listen_uri = new URL(
            process.env.LISTEN ?? config('listen') ??
            `tcp://${process.env.IP ?? config('ip') ?? 'localhost'}` +
